...
本文件描述產品合約、正式流程與可設定選項，不代表任一主機此刻的開關值。實際 `AUTO_PUBLISH`、影片可見度、訂閱者通知、timer、release 與候選批次狀態必須從私有營運 runbook 與 live state 核對；不得從範例 env 或一支測試片反推。
...
選片會檢查每檔當日 3° 內的所有精確組態，再固定依「可發布完整度 → 樣本數 → orb → signature」選出歷史證據最完整的一組；不按歷史報酬幅度挑盤。前 60 檔若湊不出五個合格分類，候選池依序擴到 100、200 檔，證據門檻不會跟著放寬。仍不足時，該交易日停止生成，不建立 `ready` 批次。

## 成片內容合約

每支影片固定七個場景：開場、五檔、結尾。五檔各佔一個且只能佔一個分類：
...
1. 當日行情日期、公司代號、公司名稱與 fact manifest 一致，且當日未還原收盤價及漲跌幅可計算。
2. 當日 3° 內存在可對回研究 signature 的精確主要相位；不能拿另一組歷史盤替代。
3. D+20 同組態完整樣本至少 5 筆，統計值能由逐筆案例重算，且至少各有 1 筆正變動與 1 筆負變動。
4. 中位數、第一四分位、第三四分位、逆向偏離與最差逆向偏離皆完整；任何必填結果若會顯示「樣本不足」，該檔不能進片。
5. 價格涵蓋範圍、原始未還原基準與研究連結完整。湊不滿五檔時停止整批，不用弱資料補位。
...
- 當下精確組態與 orb；
- D+20 樣本總數、上行筆數、下行筆數；
- D+20 中位數，或能表達分歧的第一至第三四分位範圍。

「上行筆數」只描述歷史案例，不寫成勝率。正、負、持平與混合案例都保留；歷史中位數與今天方向相反時可以當作敘事張力，但不能改寫成反轉預測。

## 鉤子、文案與聲音

開場三秒內先落一個可驗證的反差：今天與同盤中位數反向、量先醒但價格未動、同盤四分位明顯分岔，或多組相位同時擠進 3°。鉤子必須引用 manifest 內的真實數字；不用「你一定想不到」、保證、預言或買賣暗示撐張力。

五段口播不照同一個句型排隊。市場異動先講今日幅度，量能異常先講量價關係，相位密集先講交會數，歷史分歧先講四分位裂口，今昔反差先講方向差。數據段保持短句與自然轉折，讓聽者先聽懂「今天發生什麼」，再聽「歷史同盤怎麼走」。

預設聲音設定：

```dotenv
STUDIO_TTS_VOICE=onyx
STUDIO_TTS_SPEED=1.0
```

聲線為成年男性、中低音、臺灣華語、近距離收音；胸腔共鳴厚而乾淨，帶一點氣聲與克制微笑。性感來自沉著、自信與留白，不靠刻意壓嗓。開場稍輕、數據俐落、句尾自然下沉；每一到兩句要有細微節奏變化。禁止油膩低音、整段耳語、鼻音、聲帶摩擦、新聞播報腔、廣告腔、預言腔與機械式等速。

渲染會依七段實際語音長度安排場景，只在必要時做輕微加速；後製 speech rate 上限為 `1.08`，不能為硬塞文案而超速。聲音處理固定包含 65 Hz high-pass、120 Hz `+2 dB`、280 Hz `-1 dB`、3.2 kHz `+0.8 dB`、輕壓縮與 `-16 LUFS` loudness normalization，輸出 48 kHz stereo。生成乾聲，不加混響。

## 影音檢核合約

正式成片固定為 `1080 × 1920`、30 fps、MP4、H.264、AAC、`yuv420p`、48 kHz，片長 45–90 秒，且必須正好七個場景。整體響度檢核接受 `-20` 至 `-12 LUFS`，true peak 不得高於 `-1 dBFS`。縮圖為 `1080 × 1920` PNG 且不超過平台大小上限。

QC 會完整解碼影音、核對尺寸、編碼、音軌、取樣率、片長、場景數、響度、縮圖與檔案大小，並把成片、縮圖、content manifest 與 QC 以 SHA-256 綁定。任一項失敗即停止，不進入 `ready` 或發布佇列。
...
YouTube 平台分類以 `YOUTUBE_CATEGORY_NAME=Education` 查找當地可用 category。片內五檔另標示「市場異動、量能異常、相位密集、歷史分歧、今昔反差」等入選分類。這些分類只回答為什麼值得回看，不代表方向、評級或推薦。
...
- 每個數字都有資料來源；未達描述門檻或缺少上、下行任一方向的歷史案例時，整檔不進影片。
