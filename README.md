...
盤勢是一個「企業命盤 × 價格時間線」的文化研究與資料探索工具。它把上市公司的成立日或首日交易當成命盤基準，將主要行運窗口對齊實際歷史收盤價。使用者也能指定一個日期，查看同組態歷史、公司事件與資料界線，再留下可回看的研究記錄。
...
- 「問盤」流程：目標日期、命盤基準與 5 / 20 / 60 個交易日歷史對照期。
...
- 匿名查詢帳本：今日五盤不扣額度；免費版每天再查 3 檔不同股票，同一檔重查不重扣。問題條件會寫入 SQLite，安裝識別碼只保存不可逆雜湊。

## 免費版與盤勢 Pro

免費版每天可閱讀今日五盤，另外查 3 檔不同股票；公司盤、基本解讀、日期問盤摘要與觀察簿都能持續使用，營運上以廣告支持。較長的獎勵式影片由使用者自願觀看，可暫時展開一份完整歷史檔案。

盤勢 Pro 取消每日檔數限制，提供完整歷史案例、分布與不中斷的研究工作流，並移除廣告。符合資格的新訂閱者可試用 7 天；取消或到期後回到每日五盤加 3 檔額外查詢，不刪除本機觀察簿。完整方案與廣告節奏見 [docs/business-model.md](docs/business-model.md)。
...
免責文字本身不能取代法規審查。廣告或訂閱可能構成直接或間接報酬，個別公司歷史對照與今日五盤必須在臺灣公開營運前完成具體法律確認；產品邊界與上架閘門見 [docs/legal/product-boundary-tw.md](docs/legal/product-boundary-tw.md)。
